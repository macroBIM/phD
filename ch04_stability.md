...
**허용오차 두 개의 정당화.** 정지 조건에는 파라미터가 하나가 아니라 둘이다(3.1 정지 조건).

| 기호 | 값 | 출처 | 정당화 방향 |
|---|---|---|---|
| $\varepsilon_v$ | `CONVERGE` $=0.2$ | `domain.js:8` | 세그먼트 $L^1$ 속도합. 노드 수에 따라 실효값이 변하므로 **노드당**으로 환산해 정의를 고쳐야 한다 |
| $\varepsilon_p$ | $1.0$ mm | `physics.js:291` 에 직접 박힘 | 시공허용오차(KS/BS)보다 작을 것 — 하한. 반복횟수–정확도 곡선의 knee — 상한 |

$\varepsilon_p$ 가 설정에 없이 코드에 박혀 있다는 점 자체가 1.3절의 "매직넘버로 남기지 않는다"
원칙에 걸린다. 두 값을 함께 `CONFIG` 로 올리고 근거를 붙이는 것이 선행 작업이다.

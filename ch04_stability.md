...
방향이 $53^\circ$ 어긋나고 크기가 1.67배 작다.** 같은 $1/\cos$ 이 3.4절의 후퇴량
($\delta/\cos$, 철근 반지름과 적층)과 구현의 폴백 임계 $|\cos| > 0.2$(`physics.js:213`)에도
나타난다 — 임계 0.2 에서는
...
**heavy-ball 과 같다.** 위 두 식을 위치에 대해 정리하면 $\alpha = DK$, $\beta = D$ 인
**Polyak heavy-ball 경사하강**과 정확히 일치한다(3.1 정의 3.5). 따라서 아래 안정조건은
heavy-ball 의 안정조건이며 최적화 문헌의 결과를 그대로 끌어올 수 있다 — 목적함수가 스텝마다
바뀐다는 단서(4.3)를 붙이는 조건에서.

### 정리 4.2 (Jury 안정조건)

특성다항식 $p(\lambda) = \lambda^2 - (\operatorname{tr}A)\lambda + \det A$ 에 Jury 판정을 적용한다.

| 조건 | 전개 | 결과 |
|---|---|---|
| $\lvert \det A\rvert < 1$ | $D < 1$ | $0 < D < 1$ |
| $p(1) > 0$ | $1 - \operatorname{tr}A + \det A = DK$ | $K > 0$ |
| $p(-1) > 0$ | $1 + \operatorname{tr}A + \det A = 2 + 2D - DK$ | $K < \dfrac{2(1+D)}{D}$ |

$$\boxed{\;0 < D < 1, \qquad 0 < K < \frac{2(1+D)}{D}\;}$$

$D = 0.80$ 이면 $K < 4.50$ 이다. **현재값 $K = 0.08$ 은 한계의 1.8 %** 로, 안정성은 여유가 크다.
경계를 수치로 확인했다 — $K=4.4$ 에서 수렴, $K=4.6$ 에서 발산.

### 따름정리 4.3 (수렴 속도는 $K$ 가 아니라 $D$ 가 정한다)

판별식 $(\operatorname{tr}A)^2 - 4\det A < 0$ 인 진동 영역에서 고유값은 켤레복소수이므로

$$\lvert \lambda \rvert = \sqrt{\det A} = \sqrt{D}$$

즉 **감쇠율은 $K$ 와 무관**하다($D=0.8$ 이면 스텝당 $0.894$). $K$ 가 바꾸는 것은 오버슈트와
정지까지의 스텝 수뿐이다. 판별식이 0 이 되는

$$K_c = \frac{(1-\sqrt{D})^{2}}{D} \;=\; 0.0139 \quad (D=0.8)$$

이 임계감쇠이며, $K > K_c$ 는 부족감쇠다.

| $K$ | 거동 | 오버슈트 | 정지 스텝 |
|---|---|---|---|
| $0.0139 = K_c$ | 비진동 | 0.00 mm | 76 |
| 0.03 | 진동 | 5.39 mm | 44 |
| **0.08 (현재)** | 진동 | **23.82 mm** | 46 |
| 0.30 | 진동 | 51.23 mm | 51 |
| 4.50 | — | 안정 한계 | — |

(초기 남은거리 100 mm, $\varepsilon =$ `CONVERGE` $=0.2$ mm 기준.)

**짚어야 할 점.** 현재값은 이완 도중 목표를 **23.8 mm 지나친다**. 최종 위치는 정상이지만 그
동안 철근 중심이 피복선 안쪽에 있다. 최종 상태만 쓰면 무해하나, 이완 중간 상태를 읽는 로직
(적층 `stackAt` 갱신, 간섭 판정)이 있다면 실제 문제가 된다. 확인이 필요하다.

> 남은 것: $\varepsilon$ 을 절대값이 아니라 $\sqrt{D}$ 감쇠율과 묶어 "몇 스텝 뒤 정지"로
> 환산하는 정리. 그리고 목표가 바뀌는 경우(4.3)로의 확장 — 위 결과는 모두 (A1) 위에 있다.

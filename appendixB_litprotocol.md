...
## B.10 R1 추가 검색 (같은 날) — 네 칸이 모두 찼다

시공성·학위논문 방향으로 4회 더 검색해 4편을 추가했다(31편). 판정이 바뀌었다.

| 주장 칸 | R1 최초 | 추가 검색 후 |
|---|---|---|
| 장으로 위치를 생성 | 비어 있음 | 비어 있음 |
| 위치를 역학으로 정당화 | 6편 | **8편** |
| 생성 시 회피로 위치 결정 | 2편 | **3편** |
| 역학 근거 + 생성 시 회피 | **비어 있음** | **1편 — Optimizing Reinforcement Layout in Concrete Design Considering Constructability** |

**마지막 빈 칸도 찼다.** 위상최적화 계열이 시공성을 이미 결합하고 있다는 문헌이 나왔고,
"Considering Constructability in Structural Topology Optimization" 도 같은 방향이다.
이 두 편의 전문 확인이 **최우선 과제**다 — 무엇을 시공성으로 셌는지(지름·간격·정착만인지,
간섭체와 임의 위상까지인지)에 따라 남은 자리의 크기가 정해진다.

### 해석 — 문헌이 적은 분야가 아니다

이 주제 주변은 **붐비는 분야**다. 근거는 셋이다.

1. 간섭 자동화 연구가 2019–2025 로 끊이지 않고 나온다(Q-learning 2020, GAN+MARL 2019,
   2단계 GA 2021, GNN+EGA 2022, explainable GNN 2025). 최신 연구가 여전히 최상위 저널에
   실린다 — **수요가 살아 있다는 신호**다.
2. 문헌이 문제를 직접 지목한다 — *rebar clash and congestion are inevitable in complex
   joints, which impedes automatic design*.
3. 위상최적화 계열이 자기 한계를 명시한다 — *neglects constructability and rebar detailing,
   which is often the governing cost*. 이는 곧 **남의 future work 이 이미 이 자리를
   가리키고 있다**는 뜻이다.

따라서 "아무도 안 해서 비어 있다"도, "가치가 없어서 비어 있다"도 아니다.
**여럿이 각자의 절반씩 하고 있고, 합치는 시도가 이제 막 시작되는 자리**다.

### 그래서 문헌 부재로는 논문을 세울 수 없다

축을 좁게 자르면 빈 칸은 언제든 만들 수 있다. 이 부록의 네 칸도 **저자가 정의한 축**이며,
심사에서 "그건 gap 이 아니라 당신이 만든 칸"이라는 지적을 받을 수 있다.
빈 칸이 의미를 가지려면 다음 중 하나가 성립해야 한다.

1. 기존 방법이 **구체적으로 실패하는 사례**가 있고, 제안 방법이 그것을 푼다.
2. 같은 사례에서 **정량적으로 더 나은 결과**를 낸다.

**1.2 의 gap 문장을 "없다"에서 "기존 방법이 이 경우에 실패한다"로 바꾸어야 한다.**
그것이 문헌 수와 무관하게 성립하는 유일한 근거다.

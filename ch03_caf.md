...
### 정의 3.2b (역할 라벨과 확장 필터)

정의 3.2의 필터는 **방향만** 본다. 그러면 방향이 맞는 한 콘크리트면과 덕트면이 구별되지
않는다. 그러나 같은 덕트가 철근에 따라 정반대 역할을 한다.
...
따라서 경계 조각과 철근 양쪽에 역할 라벨을 둔다.

$$\rho(\gamma) \in \{\text{콘크리트면},\ \text{덕트면},\ \text{기배근}\},
\qquad \sigma(j) \in \{\text{주철근},\ \text{지지철근}\}$$

$$A(\mathbf{n}, \sigma) = \{\, \gamma \in \Gamma_{\text{cover}} \;:\;
\mathbf{n}_\gamma \cdot \mathbf{n} \le \alpha_{\text{th}}
\;\wedge\; \chi(\rho(\gamma), \sigma) = +1 \,\}$$

여기서 $\chi \in \{+1, -1, 0\}$ 은 인력·척력·무시를 정하는 **역할 결합표**다. 필터가
방향 하나에서 **방향 × 역할** 두 축으로 넓어지며, 3.2절 H1 의 well-definedness 도 그만큼
확장되어야 한다 — (W1) 비공집합 조건이 이제 역할별로 요구된다.

**본 논문의 범위.** 라벨과 결합표까지 정의하되, $\chi = -1$ 쪽(지지철근이 덕트에 인력되는
경우)의 증명과 구현은 하지 않는다. 그것이 9.2절의 RAF 이며, 여기서는 **CAF 가 $\chi=+1$
부분집합 위의 이론임을 명시하는 데** 그친다. 이 구분 없이는 5.1절의 "덕트를 경계로 편입한다"가
어느 철근에 대한 경계인지 말할 수 없어 정의가 서지 않는다.
...
### 역학적 근거 — 인력항은 무엇의 대리인가

지금까지 $U_{\text{attr}}$ 는 "스프링"이라는 비유로만 정당화되었다. 그러나 이 인력에는 역학적
의미가 있으며, 그것을 밝히는 것이 CAF 를 임의의 기하 규칙과 가르는 지점이다.

휨부재의 공칭 휨강도는 모든 인장철근이 항복한다는 가정 하에

$$M_n = \sum_i A_{s,i}\, f_y \Big(d_i - \frac{a}{2}\Big)
     = f_y\Big[\sum_i A_{s,i}\,d_i \;-\; \frac{a}{2}\sum_i A_{s,i}\Big]$$

이다. **총 강재량 $\sum A_{s,i}$ 가 주어지면 $a$ 가 고정되므로, $M_n$ 을 최대화하는 것은
$\sum A_{s,i} d_i$ 를 최대화하는 것과 같다.** 즉 철근을 인장연단 쪽으로 밀어붙이는 것은
모멘트 팔을 벌리는 일이고, 피복 $c_w$ 는 그것을 막는 하한(내구성·부착·내화)이다.

$$\underbrace{\text{피복선으로 끌어당김}}_{U_{\text{attr}}}
\;\equiv\;
\underbrace{\text{유효깊이 } d \text{ 최대화}}_{\text{모멘트 팔}}
\quad \text{s.t.} \quad \underbrace{\text{피복 } \ge c_w}_{\text{하한}}$$

**따라서 $U_{\text{attr}}$ 는 "인장연단에 최대한 붙이되 피복은 지켜라"는 설계 원리의 장(field)
표현이다.** 3.4의 $r = 0$ 이 곧 그 원리를 만족하는 유일한 점이라는 것이 명제 3.2 였다.

**단서 두 가지.**

- 위 등가는 **모든 인장철근이 항복**한다는 가정 위에 있다. 층이 깊어져 하위 층이 항복하지
  못하면 가중치가 $f_y$ 가 아니라 $f_s < f_y$ 로 바뀌어 목적함수가 달라진다. 변형률 적합을
  포함한 확장은 다루지 않는다.
- **전단철근(스터럽)은 간격 제한이 지배**하므로 이 목적과 다르다. 본 논문의 역학적 해석은
  인장 주철근에 한정한다.

이 해석이 5.3절에서 적층 배치를 최적화 문제로 세우는 근거가 된다.
